...
export default function Checkout() {
	...
	useEffect(() => {
		...
		fetch('api/products?ids=' + uniqIds.join(',')).then(response => response.json())
		.then(json => setProductsInfos(json));
	}, [ selectedProducts ])

	function moreOfProducts(id) {
		setSelectedProducts(prev => [ ...prev, id ]);
	}

	function lessOfProducts(id) {
	...
	return (
		<Layout>
				...
				<div className="flex mb-5" key={ productInfo._id }>
					...
					<div className="pl-4">
						...
						<div className="flex">
							...
							<div>
								<button onClick={ () => lessOfProducts(productInfo._id) } className="border border-emerald-500 px-2 rounded-lg text-emerald-500">-</button>
								<span className="px-2">
									{ selectedProducts.filter(id => id === productInfo._id).length }
								</span>
								<button onClick={ () => moreOfProducts(productInfo._id) } className="bg-emerald-500 px-2 rounded-lg text-white">+</button>
							</div>
						</div>
					</div>
				</div>
			...
			<form action="/api/checkout" method="POST">
				...
				<div className="mt-4">
					<input name="address" value={ address } onChange={ e => setAddress(e.target.value) } className="bg-gray-100 w-full rounded-lg px-4 py-2 mb-2" type="text" placeholder="Stree address, number" />
					...
					<input name="name" value={ name } onChange={ e => setName(e.target.value) } className="bg-gray-100 w-full rounded-lg px-4 py-2 mb-2" type="text" placeholder="You Name" />
				...
				<div className="mt-4">
					<div className="flex mt-3">
						<div className="grow font-bold text-gray-400">Subtotal:</div>
